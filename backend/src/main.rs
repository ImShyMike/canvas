...
impl Canvas {
    fn new(width: usize, height: usize) -> Self {
        Canvas {
            width,
            height,
            pixels: vec![0xFFFFFF; width * height], // fully white canvas
        }
    }
...
enum ResponseType {
    Broadcast { x: u16, y: u16, color: u32, skip_sender: bool },
    GetAllPixels(Vec<u32>),
    PixelColor(u32),
    Stats { client_count: u32, requests_per_second: f32 },
    Error(ErrorCode),
}
...
fn calculate_requests_per_second(request_tracker: &RequestTracker) -> f32 {
    let now = Instant::now();
    let interval_start = now - STATS_INTERVAL;
...
fn parse_message(canvas: &SharedCanvas, clients: &Clients, request_tracker: &RequestTracker, data: &Bytes) -> ResponseType {
    ...
    match MessageType::try_from(msg_type) {
        ...
        Ok(MessageType::GetStats) => {
            ...
            ResponseType::Stats { client_count, requests_per_second }
        }
...
async fn main() {
    ...
    let listener = TcpListener::bind("127.0.0.1:2325").await.unwrap();
        ...
        tokio::spawn(async move {
            while let Some(Ok(msg)) = ws_receiver.next().await {
                if let Message::Binary(data) = msg {
                    let action = parse_message(&canvas_ref, &clients_ref, &request_tracker_ref, &data);
